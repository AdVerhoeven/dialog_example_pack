# Called when Rejecting a request
...
execute store result score #this playerid run scoreboard players get @s delete_tp_to
tellraw @s [{translate:"essentials.request.deleted.to",fallback:"Removed request to teleport \"%s\" here",color:red,with:[{selector:"@e[predicate=essentials:match_id,limit=1]",color:white}]}]
