...
tellraw @s {translate: "essentials.request.accepted.here", fallback: 'Teleporting "%1$s" to here because they have ID: %2$s', with: [{selector: "@n[predicate=essentials:match_id]"}, {score: {name: "*", objective: "accept_tp_to"}}]}
